...
def javabha(update, context):
	...
	elif date["last command"] == "add t":
		date["temp"] = [text]
		update.message.reply_text("Enter the name of the transaction:", quote=True)
		date["last command"] = "add t 1"
	elif date["last command"] == "add t 1":
	...
	elif date["last command"] == "Checkout":
		text = text.split()
	...
	elif date["last command"] == "state 2":
...
def start(update, context):
	update.message.reply_text("Hello my frind.\nWelcome To My Bot.\nUse /help command To get more help.", quote=True)
